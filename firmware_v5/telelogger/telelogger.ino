...
byte radar_area_flag = 0x1B9;
byte cloud_qtd_flag = 0x1BA;
byte teda_time_flag = 0x1BB;
byte autocloud_time_flag = 0x1BC;
byte total_time_flag = 0x1BD;
byte cloud_index_flag = 0x1BE;
byte reset_flag = 0x1BF;
...
void processOBD(CBuffer* buffer)
{
  ...
  struct timeval teda_start, teda_end, autocloud_start, autocloud_end, total_start, total_end;
    ...
    if (obd.readPID(pid, value)) {
        ...
        if (count == 4){
          ...
          if (flag == 0 || outlier_detection == 4){
            ...
            // end
            gettimeofday(&autocloud_end, NULL);

            buffer->add(cloud_index_flag, ELEMENT_INT32, &driver_index, sizeof(driver_index));
            ...
            // Serial.println(autocloud_time);
          } else {
            float autocloud_time = 0.0;
            outlier_detection++;
            int driver_index = -1;
            buffer->add(cloud_index_flag, ELEMENT_INT32, &driver_index, sizeof(driver_index));
            buffer->add(autocloud_time_flag, ELEMENT_FLOAT, &autocloud_time, sizeof(autocloud_time));
          ...
          if (outlier_detection == 4){
            ...
            reset += 1;
            // log reset
            buffer->add(reset_flag, ELEMENT_INT32, &reset, sizeof(reset));
          }
          ...
          // calculate time
          float teda_time = (teda_end.tv_sec - teda_start.tv_sec) + (teda_end.tv_usec - teda_start.tv_usec) / 1000000.0;
          float total_time = (total_end.tv_sec - total_start.tv_sec) + (total_end.tv_usec - total_start.tv_usec) / 1000000.0;

          buffer->add(teda_time_flag, ELEMENT_FLOAT, &teda_time, sizeof(teda_time));
          buffer->add(total_time_flag, ELEMENT_FLOAT, &total_time, sizeof(total_time));
        }
